    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-4">
            <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                <div class="flex items-center justify-between text-xs uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                    <span>Torneos activos</span>
                    <span class="material-symbols-rounded text-amber-500 text-4xl leading-none">flag</span>
                </div>
                <div class="mt-3 flex items-baseline gap-2">
                    <span class="text-3xl font-bold text-neutral-900 dark:text-neutral-100">{{ $activeTournaments }}</span>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900"
                ...
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <div class="text-xs text-neutral-500 dark:text-neutral-400">{{ $monthStart->translatedFormat('F Y') }}</div>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="{{ route('dashboard', ['month' => $prevMonth]) }}" class="rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-neutral-600 hover:border-neutral-300 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-200 dark:hover:border-neutral-600">←</a>
                        <span class="material-symbols-rounded text-indigo-500 text-3xl">event_upcoming</span>
                        <a href="{{ route('dashboard', ['month' => $nextMonth]) }}" class="rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-neutral-600 hover:border-neutral-300 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-200 dark:hover:border-neutral-600">→</a>
                    </div>
                </div>
                ...
                <div class="mt-2 grid grid-cols-7 gap-1 text-sm">
                    ...
                    @for($day = 1; $day <= $daysInMonth; $day++)
                        ...
                        <button type="button"
                            ...
                            @if($count > 0)
                                <span class="mt-1 inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2 py-0.5 text-[11px] font-semibold text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200">
                ...
                <div class="mt-6">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-xs text-neutral-500 dark:text-neutral-400" x-text="selectedMatches.length + ' partidos'"></span>
                    </div>
                    <div class="space-y-2" x-show="selectedMatches.length > 0">
                        <template x-for="match in selectedMatches" :key="match.date + match.time + match.home + match.away">
                            <div class="flex items-center justify-between rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2.5 text-sm dark:border-neutral-700 dark:bg-neutral-800/60">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
                                        <span class="material-symbols-rounded text-[14px] text-amber-500">emoji_events</span>
                                        <span x-text="match.tournament ?? 'Torneo'"></span>
                                    </div>
                                    <div class="font-semibold text-neutral-900 dark:text-neutral-100 truncate" x-text="match.home"></div>
                                    <div class="text-[11px] text-neutral-500 dark:text-neutral-400">vs</div>
                                    <div class="font-semibold text-neutral-900 dark:text-neutral-100 truncate" x-text="match.away"></div>
                                </div>
                                <div class="text-right text-xs text-neutral-600 dark:text-neutral-400 space-y-0.5">
                                    <div class="flex items-center justify-end gap-1">
                                        <span class="material-symbols-rounded text-[15px] text-indigo-500">schedule</span>
                                        <span class="font-semibold text-neutral-900 dark:text-neutral-100" x-text="match.time"></span>
                                    </div>
                                    <div class="flex items-center justify-end gap-1">
                                        <span class="material-symbols-rounded text-[15px] text-emerald-500">stadium</span>
                                        <span class="truncate" x-text="match.field ?? '-'"></span>
                                    </div>
                                    <template x-if="match.round">
                                        <div class="text-[11px] text-neutral-500 dark:text-neutral-400" x-text="match.round"></div>
                                    </template>
                                </div>
                            </div>
                        </template>
                    </div>
                    <div x-show="selectedMatches.length === 0" class="rounded-xl border border-dashed border-neutral-200 bg-neutral-50 px-3 py-2 text-sm text-neutral-500 dark:border-neutral-700 dark:bg-neutral-800/60 dark:text-neutral-400">
